...
## 9. Sistema tipográfico — tokens y roles

Toda la tipografía del proyecto se define con tokens. **No se permiten valores literales** de `font-size`, `font-weight` ni `font-family` fuera de este sistema. Si necesitas un tamaño o peso que no existe, primero discútelo y agrégalo al sistema; no lo improvises en una hoja suelta.

### 9.1 Familias

Solo dos familias, ambas son la pila SF del sistema operativo. Sin imports de Google Fonts, sin webfonts.

| Token          | Uso                                                  | Pila                                                                         |
| -------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------- |
| `--ff-display` | Títulos grandes, números hero, badges con número     | `-apple-system, BlinkMacSystemFont, "SF Pro Display", system-ui, sans-serif` |
| `--ff-text`    | Todo lo demás (body, labels, inputs, botones, hints) | `-apple-system, BlinkMacSystemFont, "SF Pro Text", system-ui, sans-serif`    |

### 9.2 Pesos

Solo tres pesos. Nada de 300, 800, 900.

| Token          | Valor | Uso                                                                  |
| -------------- | ----- | -------------------------------------------------------------------- |
| `--fw-regular` | 400   | Body, valores de input, hints                                        |
| `--fw-medium`  | 600   | Labels de campo, botones, item de lista (nombre), nav, énfasis suave |
| `--fw-bold`    | 700   | Títulos de sección, números hero, badges destacados                  |

### 9.3 Tamaños

Cuatro escalones. Si necesitas un quinto, la jerarquía está mal pensada.

| Token          | Valor | Line-height | Uso                                                                     |
| -------------- | ----- | ----------- | ----------------------------------------------------------------------- |
| `--fs-display` | 28px  | 1.2         | Título de pantalla, números hero (37/41 créditos), encabezados de modal |
| `--fs-title`   | 14px  | 1.3         | Título de panel/sección, header de tabla, header de modal               |
| `--fs-body`    | 13px  | 1.45        | Body, valores de input, texto de botones, items de lista                |
| `--fs-label`   | 11px  | 1.3         | Labels de campo (uppercase), píldoras/contadores, badges                |
| `--fs-caption` | 11px  | 1.4         | Hints, textos auxiliares, metadatos secundarios                         |

> `--fs-label` y `--fs-caption` comparten tamaño pero **no son intercambiables**: label es uppercase con tracking; caption es sentence case con line-height más amplio.

### 9.4 Tracking (letter-spacing)

| Token         | Valor   | Uso                           |
| ------------- | ------- | ----------------------------- |
| `--ls-tight`  | -0.02em | Display y títulos largos      |
| `--ls-snug`   | -0.01em | Title, body con énfasis       |
| `--ls-normal` | 0       | Body en general               |
| `--ls-wide`   | 0.06em  | Labels uppercase y contadores |

### 9.5 Roles tipográficos

Cada elemento de UI debe encajar en uno de estos roles. Elementos del mismo rol se tipografían **idénticos** en todas las pantallas.

| Rol                                                  | Familia        | Tamaño         | Peso           | Tracking      | Caso          |
| ---------------------------------------------------- | -------------- | -------------- | -------------- | ------------- | ------------- |
| **Display** — números hero, título de pantalla       | `--ff-display` | `--fs-display` | `--fw-bold`    | `--ls-tight`  | Sentence case |
| **Section title** — encabezado de panel/módulo       | `--ff-text`    | `--fs-title`   | `--fw-bold`    | `--ls-snug`   | Sentence case |
| **Field label** — nombre de input/campo              | `--ff-text`    | `--fs-label`   | `--fw-medium`  | `--ls-wide`   | UPPERCASE     |
| **Body / value** — texto de input, párrafo, item     | `--ff-text`    | `--fs-body`    | `--fw-regular` | `--ls-normal` | Sentence case |
| **Action** — texto de botón                          | `--ff-text`    | `--fs-body`    | `--fw-medium`  | `--ls-snug`   | Sentence case |
| **Item primary** — nombre principal de un chip/card  | `--ff-text`    | `--fs-body`    | `--fw-medium`  | `--ls-normal` | Sentence case |
| **Counter / pill** — contadores, conteos en píldoras | `--ff-text`    | `--fs-label`   | `--fw-bold`    | `--ls-wide`   | Tabular nums  |
| **Caption / hint** — ayuda, metadatos secundarios    | `--ff-text`    | `--fs-caption` | `--fw-regular` | `--ls-normal` | Sentence case |

### 9.6 Reglas duras

- **Cero cursivas.** Cualquier `<em>` o `<i>` no icónico convierte a peso medium del mismo tamaño.
- **Cero uppercase fuera del rol "Field label"** y "Counter/pill". Títulos, botones e items van en sentence case.
- **Números con `font-variant-numeric: tabular-nums`** en horas, créditos, contadores, IDs.
- **Antialiasing siempre activo** en cualquier contenedor de UI: `-webkit-font-smoothing: antialiased`, `-moz-osx-font-smoothing: grayscale`.
- **Bootstrap classes que tocan tipografía** (`.small`, `.text-muted`, `.fw-bold`, `.fst-italic`) están **prohibidas** en componentes nuevos. Usa los roles.
