...
MAX_CREDITOS = 41
MIN_CREDITOS = 37
MATERIA_OBLIGATORIA = "ETICA PROFESIONAL"
...
def tiene_obligatoria(lista_cursos):
    for c in lista_cursos:
        if c['asignatura'] == MATERIA_OBLIGATORIA: return True
    return False
...
def barra_creditos(actuales, tiene_etica):
    ...
    aviso_etica = "✅ ETICA" if tiene_etica else "🛑 FALTA ETICA"
...
# ==========================================
# (Se mantiene igual, funcional y robusto)
...
def modo_explorador():
    ...
    while True:
        ...
        tiene_etica = MATERIA_OBLIGATORIA in seleccion_candidatos
        aviso_etica = "✅ Incluida" if tiene_etica else "🛑 FALTA"
        
        mostrar_encabezado(f"MODO EXPLORADOR (Bolsa: {total_materias} materias)")
        print(f"   Estado {MATERIA_OBLIGATORIA}: {aviso_etica}")
        print("-" * 70)
        
        if total_materias > 0:
            for i, (nombre_mat, opciones) in enumerate(seleccion_candidatos.items()):
                # Revisar si esta anclada (solo 1 opcion seleccionada)
                estado = f"({len(opciones)} opciones)"
                if len(opciones) == 1:
                    prof = opciones[0]['profesor'][:15]
                    estado = f"⚓ ANCLADO: {prof}"
                
                tag = "⭐" if nombre_mat == MATERIA_OBLIGATORIA else ""
                print(f" {i+1}. {nombre_mat} {tag} -> {estado}")
        else:
            print(" (Bolsa vacia)")

        print("\nACCIONES:")
        print("1. [AGREGAR/EDITAR] Seleccionar Materia")
        print("2. [BORRAR]         Sacar Materia")
        ...
        if op == "3":
            if not tiene_etica:
                print(f"\n[!] ALERTA: Faltara '{MATERIA_OBLIGATORIA}'.")
                c = input("¿Continuar? (S/N): ").upper()
                if c != "S": continue
            
            procesar_combinaciones_con_anclaje(seleccion_candidatos)
        ...
        if op == "1":
            ...
            while True:
                ...
                print("   Escribe el numero para marcar/desmarcar.")
                print("   Escribe 'A <numero>' para ANCLAR (ej: 'A 2').")
                print("-" * 75)
                ...
                for cid, ses in lista_g:
                    ...
                    # Logica visual de Ancla
                    if len(mis_candidatos) == 1 and esta: marca = "⚓ ANCLA"
                    elif esta: marca = "[X]"
                    else: marca = "[ ]"
                    
                    hor = ", ".join([f"{s['dia'][:3]} {s['hora_inicio']}" for s in ses])
                    
                    print(f"{idx:<3} {marca:<7} {base['grupo']:<6} {base['profesor'][:25]:<25} {hor}")
                    mapa_local[idx] = {'id': cid, 'asignatura': mat_nom, 'grupo': base['grupo'], 'profesor': base['profesor'], 'sesiones': ses}
                    idx += 1
                
                sel_c = input("\n> Opcion (0 termina): ").upper()
                ...
                # LOGICA DE ANCLAJE
                if sel_c.startswith("A "):
                    try:
                        num_anclar = int(sel_c.split()[1])
                        if num_anclar in mapa_local:
                            # Borramos todo y dejamos solo este
                            mis_candidatos = [mapa_local[num_anclar]]
                            print("¡ANCLADO! Regresando al menu...")
...
def procesar_combinaciones_con_anclaje(diccionario_candidatos):
    if not diccionario_candidatos: return

    listas = list(diccionario_candidatos.values())
    
    print("\n" + "="*60)
    print("   CALCULANDO... (Buscando los mejores)")
    print("="*60)
        ...
        if es_valido:
            ...
            for dia, lapsos in agenda.items():
                ...
                for k in range(len(lapsos) - 1):
                    dif = lapsos[k+1][0] - lapsos[k][1]
                    if dif > 0: huecos += dif
            
            validos.append({'cursos': comb, 'huecos': huecos, 'num_dias': len(dias_set)})
            if len(validos) >= 200: break
    
    # RANKING
    validos.sort(key=lambda x: (x['num_dias'], x['huecos']))

    if len(validos) > 0:
        print(f"\n✅ ENCONTRE {len(validos)} HORARIOS VALIDOS.")
        
        # --- FILTRO POST-GENERACION (NUEVO) ---
        print("\n¿Quieres filtrar estos resultados por un profesor?")
        print("Escribe el apellido (ej: 'BACHA') o Enter para ver todos.")
        filtro_profe = input("> ").upper()
        
        resultados_finales = []
        if filtro_profe:
            for v in validos:
                # Buscar si algun profe del horario coincide con el texto
                if any(filtro_profe in c['profesor'] for c in v['cursos']):
                    resultados_finales.append(v)
            print(f"-> Filtrando... quedan {len(resultados_finales)} opciones con '{filtro_profe}'.")
        else:
            resultados_finales = validos

        # MOSTRAR TOP 15
        for i, data in enumerate(resultados_finales[:15]):
            hor = data['cursos']
            h_hr = round(data['huecos'] / 60, 1)
            dias = data['num_dias']
            c_tot = calcular_creditos(hor)
            
            tag = "✅"
            if c_tot > MAX_CREDITOS: tag = "🚫 EXCESO"
            elif c_tot < MIN_CREDITOS: tag = "⚠️ BAJO"
            
            print(f"\n--- OPCION {i+1} (Días: {dias} | Huecos: {h_hr}h | {c_tot} Cr {tag}) ---")
            for curso in hor:
                cr = curso['sesiones'][0]['creditos']
                tag_o = "⭐" if curso['asignatura'] == MATERIA_OBLIGATORIA else ""
                print(f"  * {curso['asignatura'][:20]:<20} ({cr}) {tag_o} | {curso['profesor'][:20]}")
        
        save = input("\n¿Guardar resultados? (S/N): ").upper()
        if save == "S": 
            guardar_multiples_txt([v['cursos'] for v in resultados_finales])
            
    else:
        print("\n❌ 0 HORARIOS VALIDOS. Todo choca.")
...
if __name__ == "__main__":
    while True:
        mostrar_encabezado("GENERADOR UNAM V6 (Anclaje + Filtros)")
        print("1. MODO MANUAL")
